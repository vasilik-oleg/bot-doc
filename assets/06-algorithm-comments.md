...
Target price - это цена исходной заявки, в то время как цена, по которой прошла сделка отображается в столбце Price. Target price есть только для вторых ног. Робот, как видит какую-то раздвижку, выставляет первую ногу и запоминает цены для вторых ног, которые были на момент выставления первой ноги. Когда первая нога проходит, заявки ставятся. по target price, сохраненным ценам исходной заявки.
