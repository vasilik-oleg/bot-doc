...
Если текущее локальное время на сервере (`curTime`) попадает в один из периодов, т.е. $Begin_i <= curTime <= End_i$, то `re_sell` и `re_buy` включаются автоматически (кроме режима [Timetable only stop](05-params-description.md#p.tt_only_stop)), в противном случае (если текущее время не попадает ни в один из периодов) `re_sell` и `re_buy` выключаются автоматически.

**Важно:** текущее время определяется временем на сервере, где находится робот. Для роботов, торгующих на криптовалютных биржах, это UTC, для всех остальных это UTC +3 (МСК). Текущее время на сервере есть отображается в виджете `Robots` в столбце `Robot time`
