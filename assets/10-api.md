...
в очередь перед отправкой и отправку сразу всех сообщений из очереди после истечения заданного таймаута (сообщения могут быть отправлену и не дожидаясь истечения таймаута если очередь уже достигла "большого" размера или если
...
| > group | n | number |  | Receive messages from server in groups with group timeout, default value is 0 (no grouping), available values are in range [0, 3] with step 0.01 |
