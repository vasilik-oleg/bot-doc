...
Пароль для данного FIX-подключения. Первоначально выставляется пароль переданный брокером. В случае, если от брокера не было информации о пароле, оставляем в данном поле пароль по умолчанию ( password). Как только подключение создано, робот его автоматически поменяет пароль по умолчанию на новый (генерируется рандомно). При пересоздании подключения нужен будет новый пароль, поэтому СРАЗУ ПОСЛЕ ДОБАВЛЕНИЯ необходимо перейти во вкладку Params и сохранить себе новый пароль.
