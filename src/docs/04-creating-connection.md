...
При заказе данного подключения следует учесть, что 2 логина по 30 транзакций/сек будут работать лучше, чем один на 60 транзакций/сек (см. [Round robin](/docs/05-params-description.html#_5-3-22-client-code)).

#### **4.1.1.1. Name** <Anchor :ids="['tc.MOEX_FUT_OPT.name']" />
...
#### **4.1.1.2. Credentials** <Anchor :ids="['tc.MOEX_FUT_OPT.credentials']" />

Credentials предоставляются брокером.

#### **4.1.1.3. Trading account(s)** <Anchor :ids="['tc.MOEX_FUT_OPT.client_code']" />
...
#### **4.1.1.4. Comment** <Anchor :ids="['tc.MOEX_FUT_OPT.comment']" />
...
#### **4.1.1.5. Round robin** <Anchor :ids="['tc.MOEX_FUT_OPT.round_robin']" />
...
#### **4.1.1.6. Max trans** <Anchor :ids="['tc.MOEX_FUT_OPT.max_transactions_in_one_second']" />
...
#### **4.1.1.7. Reserved trans** <Anchor :ids="['tc.MOEX_FUT_OPT.transactions_reserved']" />
...
#### **4.1.1.8. Move order** <Anchor :ids="['tc.MOEX_FUT_OPT.is_move_available']" />
...
#### **4.1.1.9. Bind IP** <Anchor :ids="['tc.MOEX_FUT_OPT.bind_ip']" />
...
#### **4.2.1.1. Name** <Anchor :ids="['tc.MOEX_FOND.name']" />

Поле для задания имени подключения. Это значение задается для удобства, чтобы потом в списке транзакционных подключений было проще ориентироваться. Разрешенные символы: `_ a-z A-Z 0-9`

#### **4.2.1.2. Server** <Anchor :ids="['tc.MOEX_FOND.host_port_target_comp_id']" />

Адрес FIX-сервера биржи, к которому будет выполняться подключение.

#### **4.2.1.3. SenderCompID** <Anchor :ids="['tc.MOEX_FOND.sender_comp_id']" />

Значение данного параметра предоставляется брокером.

#### **4.2.1.4. Password** <Anchor :ids="['tc.MOEX_FOND.password']" />
...
#### **4.2.1.5. Trading account(s)** <Anchor :ids="['tc.MOEX_FOND.client_code']" />

Торговый счёт, предоставляется брокером. Также его можно посмотреть в терминале (если счетов несколько, то они указываются через запятую):

![Alt text](./00-img/4-2-1-5.png)

#### **4.2.1.6. Round robin** <Anchor :ids="['tc.MOEX_FOND.round_robin']" />
...
#### **4.2.1.7. Client code** <Anchor :ids="['tc.MOEX_FOND.clordid']" />
...
#### **4.2.1.8. Firm level account** <Anchor :ids="['tc.MOEX_FOND.moex_party_id']" />
...
#### **4.3.1.1. Name** <Anchor :ids="['tc.MOEX_CURR.name']" />

Поле для задания имени подключения. Это значение задается для удобства, чтобы потом в списке транзакционных подключений было проще ориентироваться. Разрешенные символы: `_ a-z A-Z 0-9`.

#### **4.3.1.2. Server** <Anchor :ids="['tc.MOEX_CURR.host_port_target_comp_id']" />
...
#### **4.3.1.3. SenderCompID** <Anchor :ids="['tc.MOEX_CURR.sender_comp_id']" />

Значение данного параметра предоставляется брокером.

#### **4.3.1.4. Password** <Anchor :ids="['tc.MOEX_CURR.password']" />
...
#### **4.3.1.5. Trading account(s)** <Anchor :ids="['tc.MOEX_CURR.client_code']" />
...
#### **4.3.1.6. Round robin** <Anchor :ids="['tc.MOEX_CURR.round_robin']" />
...
#### **4.3.1.7. Client code** <Anchor :ids="['tc.MOEX_CURR.clordid']" />
...
#### **4.3.1.8. Firm level account** <Anchor :ids="['tc.MOEX_CURR.moex_party_id']" />
...
#### **4.3.1.9. Bind IP** <Anchor :ids="['tc.MOEX_CURR.bind_ip']" />
...
#### **4.4.2.1. Name** <Anchor :ids="['tc.SPB.name']" />

Поле для задания имени подключения. Это значение задается для удобства, чтобы потом в списке транзакционных подключений было проще ориентироваться. Разрешенные символы: `_ a-z A-Z 0-9`.

#### **4.4.2.2. Connection type** <Anchor :ids="['tc.SPB.conn_type']" />
...
#### **4.4.2.9. Comment** <Anchor :ids="['tc.SPB.comment']" />

Клиентский комментарий к заявкам.

#### **4.4.2.10. Bind IP** <Anchor :ids="['tc.SPB.bind_ip']" />
...
### **4.5.1. Name** <Anchor :ids="['tc.ALORMOEX.name']" />

Поле для задания имени подключения. Это значение задается для удобства, чтобы потом в списке транзакционных подключений было проще ориентироваться. Разрешенные символы: `_ a-z A-Z 0-9`

### **4.5.2. Refresh token** <Anchor :ids="['tc.ALORMOEX.refresh_token_part']" />

Токен для доступа к API.

### **4.5.3. Trading account(s)** <Anchor :ids="['tc.ALORMOEX.client_code']" />

Торговый счет клиента

## **4.6. ALOR OPEN API SPBEX**

### **4.6.1. Name** <Anchor :ids="['tc.ALORSPB.name']" />

Поле для задания имени подключения. Это значение задается для удобства, чтобы потом в списке транзакционных подключений было проще ориентироваться. Разрешенные символы: `_ a-z A-Z 0-9`

### **4.6.2. Refresh token** <Anchor :ids="['tc.ALORSPB.refresh_token_part']" />

Токен для доступа к API.

### **4.6.3. Trading account(s)** <Anchor :ids="['tc.ALORSPB.client_code']" />
...
Один из параметров системы предупреждения флуда. Чтобы не натыкаться на флуд-контроль со стороны брокера, мы сами считаем отправляемые сообщения. Этот параметр определяет количество сообщений за промежуток времени. Если количество сообщений за временной промежуток [Max trans interval](/docs/04-creating-connection.html#_4-7-7-max-trans-interval) выше данного значения, то робот перестаёт отправлять сообщения брокеру.

### **4.7.7. Max trans interval**

Один из параметров системы предупреждения флуда. Чтобы не натыкаться на флуд-контроль со стороны брокера, мы сами считаем отправляемые сообщения. Этот параметр определяет промежуток времени. Если количество сообщений за этот временной промежуток выше значения [Max trans](/docs/04-creating-connection.html#_4-7-6-max-trans), то робот перестаёт отправлять сообщения брокеру.

### **4.7.8. Transactions reserved**

Один из параметров системы предупреждения флуда. Этот параметр определяет сколько транзакций зарезервировано под снятие заявок во временной промежуток [Max trans interval](/docs/04-creating-connection.html#_4-7-7-max-trans-interval). Т.е. даже если выставление заявок будет нарываться на внутренний флуд контроль робота, робот сможет отправить заявки на снятие в количестве Transactions reserved штук.
...
Параметр появлется после выставления галочки [Add market-data connection](/docs/04-creating-connection.html#_4-8-8-add-market-data-connection)
...
### **4.17.5. Create fast data connection**
...
#### **4.19.2.1. Поля для подключения FIX**
...
##### **4.19.2.1.2. Exchange account id**
...
#### **4.19.2.2. Поля для подключения WEBSOCKET**
...
### **4.26.2. Public key id**

Публичный ключ для доступа к API биржи, создаётся в личном кабинете на сайте биржи. Создаётся вместе с соотвествующим секретным ключом. Находится чаще всего по пути Настройки - Безопасность - API.

### **4.26.3. Access secret**
