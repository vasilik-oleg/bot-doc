...
Пароль для данного FIX-подключения. Первоначально выставляется тот, который передала биржа. Но как только подключение создано, робот его автоматически меняет на новый (генерируется рандомно). При пересоздании подключения нужен будет новый пароль, поэтому СРАЗУ ПОСЛЕ ДОБАВЛЕНИЯ необходимо перейти во вкладку Params и сохранить себе новый пароль.
...
### **4.6.7. Securities dictionary**

Словарь соответствия имён тикеров EXANTE:CQG. Заполняется в соответствии с примером.

### **4.6.8. Add market-data connection**

Необходимо выставить галочку в чек-боксе

### **4.6.9. Market-data key**

Параметр появлется после высавления галочки [Add market-data connection](/docs/04-creating-connection.html#_4-6-8-ticker-dictionary)
