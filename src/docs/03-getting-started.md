...
Для активации маркет-дата подключений необходимо выбрать виджет `Data Connections`, отметить галочками необходимые потоки маркет-даты, нажать значок шестеренка <img src="./00-img/icons/settings_black.svg" width="16" height="16"/>, выбрать робота, для которого добавляется подключение и выбрать действие `Enable`.
