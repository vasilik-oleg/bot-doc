...
где tradePrice<sub>i</sub> - цена сделки, а tradeAmount<sub>i</sub> - количество лотов в сделке, bought - список сделок на покупку, sold - список сделок на продажу, Multi - Fin res multiplier инструмента портфеля.
...
`Sell` – расчетная цена на продажу. Нередактируемый параметр.<br>
`Buy` – расчетная цена на покупку. Нередактируемый параметр.<br>
...
`Price_s` – цена выставления заявки на продажу по [Is first 5.3.11]() бумаге, вычисляется как обратная функция для [Sell 5.2.39](), где цена [Sell 5.2.39]() заменяется на [Lim_Sell 5.2.18](). Нередактируемый параметр.<br>
...
Статус заявки на продажу/покупку по [Is first 5.3.11]() бумаге (не используется при выставлении заявки с помощью "кликеров").<br>
...
Оборот по [Is first 5.3.11]() бумаге, вычисляется с момента старта серверной части робота как сумма модулей количества лотов в сделках по [Is first 5.3.11]() инструменту.
...
где secBid<sub>i</sub> - лучшая цена на покупку инструмента портфеля, secOffer<sub>i</sub> - лучшая цена на продажу инструмента портфеля, Curpos<sub>i</sub> - текущая позиция инструмента портфеля, Mult - [Fin res multiplier 5.3.21]() инструмента портфеля, secs - список инструментов портфеля.

### **5.2.45. Fin res wo C**

`Fin res` без учета комиссии. Вычисляется по формуле:

![Alt text](../00-img/5-2-45.jpg)
